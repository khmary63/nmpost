...
serve(async (req) => {
  ...
  try {
    ...
    const { prompt, style, type, originalText, postsCount, periodDays, toneSample } = await req.json();
    ...
    const decorateInstructions: Record<string, string> = {
      ...
      elegant: "Очень сдержанный декор. Один утончённый эмодзи (✦ ✧ ◆ ❖) только в самом начале поста и, при необходимости, ещё один у финального призыва — больше нигде. Заголовок (первая строка) — жирным **текст**. Курсив *текст* использовать ИЗРЕДКА — максимум 1-2 коротких выражения на весь пост (например, цитата или ключевой термин), НИКОГДА не оформлять курсивом целые предложения или абзацы. Между смысловыми блоками — тонкий разделитель ─────────────.",
...
Возвращай ТОЛЬКО текст поста, без пояснений.`;
    ...
    // Tone of Voice — образец личного стиля автора
    if (toneSample && typeof toneSample === "string" && toneSample.trim().length > 0 && type !== "decorate") {
      const sample = toneSample.trim().slice(0, 4000);
      systemPrompt += `

ВАЖНО — TONE OF VOICE АВТОРА:
Ниже приведён пример поста, написанного лично автором. Используй его как ОРИЕНТИР по интонации, лексике, ритму, длине предложений, манере подачи и характерным словечкам.
- НЕ копируй тему и не повторяй конкретные факты из образца — пиши на тему пользователя.
- Сохрани правила выбранного стиля оформления (декор, эмодзи, форматирование), но СЛОГ и ИНТОНАЦИЯ должны звучать как у автора образца.
- Если у автора короткие рубленые фразы — пиши коротко. Если длинные размышления — длинно. Если разговорно — разговорно.

ОБРАЗЕЦ АВТОРА (только для имитации стиля, не темы):
"""
${sample}
"""`;
    }
